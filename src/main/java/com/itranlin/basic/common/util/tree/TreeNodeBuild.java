...
public class TreeNodeBuild {
    ...
    public static <S extends SourceNode<ID>, T extends TargetNode<T, ID>, ID> List<T> buildForest(
            List<S> sourceList, NodeTransform<S, T, ID> transform, List<ID> rootIds, Comparator<T> sortComparator) {
        ...
        rootIds.forEach(id -> rootList.add(sourceNodeMap.get(id)));
        if (null != sortComparator) {
     ...
     */
    public static <S extends SourceNode<ID>, T extends TargetNode<T, ID>, ID> List<T> buildForest(
            List<S> sourceList, NodeTransform<S, T, ID> transform, List<ID> rootIds) {
        return buildForest(sourceList, transform, rootIds, null);
    }
